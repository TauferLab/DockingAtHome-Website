...
if ($userid) {
    ...
    if ($user->show_hosts) {
        display_page_top("Computers belonging to $user->name");
        more_or_less($show_all);
        user_host_table_start(false);
    } else {
        display_page_top("Computers hidden");
    ...
    $caching = false;
    $userid = $user->id;
    display_page_top("Your computers");
    more_or_less($show_all);
...
if ($private) {
    echo "
        <a href=/community/merge_by_name.php>Merge computers by name</a>
    ";
}
