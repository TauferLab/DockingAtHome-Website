...
function daemon_status($host, $pidname, $progname, $disabled) {
    ...
    $path = $_SERVER['DOCUMENT_ROOT']."/../../pid_$host/$pidname.pid";
    $running = 0;
    if (is_file($path)) {
...
$version = null;
if (file_exists($_SERVER['DOCUMENT_ROOT']."/../../local.revision")) {
    $version = trim(file_get_contents($_SERVER['DOCUMENT_ROOT']."/../../local.revision"));
}
$now = time();
...
if ($xml) {
    ...
    display_page_top("Docking@Home Server Status");
	echo "\n";
	echo "<p>This page shows the current status of the Docking@Home servers.</p>\n";
    if ($version) {
        echo "<p>Server software version: $version</p>\n";
    }
    echo "<p>".time_str(time())."</p>\n";
    echo "
		<table width=\"100%\">
        <tr>
        ...
        <tr><th align=\"left\">Program</th><th align=\"left\">Host</th><th align=\"left\">Status</th></tr>
    ";
}
;
...
//
$web_running = !file_exists($_SERVER['DOCUMENT_ROOT']."/../../stop_web");
...
$sched_running = !file_exists($_SERVER['DOCUMENT_ROOT']."/../../stop_sched");
...
if ($retval) {
    ...
    if (!$xml) {
        echo "
            <table border=\"0\" cellpadding=\"4\">
            <tr><th align=\"left\">State</th><th align=\"left\">#</th></tr>
        ";
    }
